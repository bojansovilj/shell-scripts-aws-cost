...
- Service-specific read permissions (e.g., `lambda:ListFunctions`, `s3:ListBuckets`, `redshift:DescribeClusters`, `redshift:DescribeReservedNodes`, `ec2:DescribeInstances`, `ec2:DescribeReservedInstances`, `dynamodb:ListTables`, `dynamodb:DescribeTable`, `kinesis:ListStreams`, `kinesis:DescribeStream`, `firehose:ListDeliveryStreams`)
...
chmod +x kinesis/check_kinesis_costs.sh
# ... etc
```
...
| `kinesis/check_kinesis_costs.sh` | Kinesis service costs | Shard hours, data ingestion, KPU usage |
...
### 8. Kinesis Cost Analysis

```bash
# Check Kinesis service costs
./kinesis/check_kinesis_costs.sh --profile production us-east-1
```

**Sample Output:**
```
Kinesis Services Analysis - Region: us-east-1 (Last Month: 2024-01-01 to 2024-02-01)
Actual Total Cost from Cost Explorer: $156.78

Kinesis Cost Breakdown by Service:
---------------------------------
Amazon Kinesis                                   $89.2340
Amazon Kinesis Firehose                          $45.6700
Amazon Kinesis Analytics                         $21.8760

Kinesis Data Streams Analysis:
=============================
+------------------------------------+-------+--------+---------------+----------------+-------------+
| Stream Name                        | Shards| Status | Retention     | Encryption     | Est. Price  |
+------------------------------------+-------+--------+---------------+----------------+-------------+
| user-activity-stream               | 4     | ACTIVE | 24h (Standard)| KMS            | $43.20      |
| transaction-events                 | 2     | ACTIVE | 168h (Extended)| None          | $75.60      |
| analytics-pipeline                 | 6     | ACTIVE | 24h (Standard)| KMS            | $64.80      |
+------------------------------------+-------+--------+---------------+----------------+-------------+

Kinesis Data Firehose Analysis:
==============================
+------------------------------------+--------+---------------+----------------+-------------+
| Delivery Stream                    | Status | Destination   | Compression    | Est. Price  |
+------------------------------------+--------+---------------+----------------+-------------+
| logs-to-s3                         | ACTIVE | S3/Other      | GZIP           | $Variable   |
| events-to-redshift                 | ACTIVE | S3/Other      | None           | $Variable   |
+------------------------------------+--------+---------------+----------------+-------------+
```

**What it shows:**
- Actual Kinesis costs from Cost Explorer across all services
- Cost breakdown by service type (Streams, Firehose, Analytics)
- Data Streams with shard counts, retention periods, and encryption
- Estimated costs based on shard hours ($0.015/shard/hour)
- Firehose delivery streams with compression settings
- Variable costs for Firehose based on data volume

### 9. NAT Gateway Cost Analysis
...
### 10. Data Transfer Cost Analysis
...
- Use data compression in Firehose to reduce ingestion costs
- Monitor shard utilization and merge under-utilized shards
- Set appropriate data retention periods (default 24h is often sufficient)
